...
public class AliOssWrapper implements ImageUploader, InitializingBean, DisposableBean {
    ...
    public String upload(byte[] bytes, String fileType) {
        ...
        try {
            ...
            PutObjectRequest putObjectRequest =
                    new PutObjectRequest(properties.getOss().getBucket(), fileName, input);
    ...
    public boolean uploadIgnore(String fileUrl) {
        if (StringUtils.isNotBlank(properties.getOss().getHost())
                && fileUrl.startsWith(properties.getOss().getHost())) {
            return true;
        }
        //外链保存
