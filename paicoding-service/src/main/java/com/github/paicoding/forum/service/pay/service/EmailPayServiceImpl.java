...
public class EmailPayServiceImpl implements PayService {
    ...
    public boolean paying(ArticlePayRecordDO record) {
        ...
        try {
            record.setVerifyCode(
                    IdUtil.genPayCode(
                            ThirdPayWayEnum.ofPay(
                                    record.getPayWay())
                            , record.getId()));

            //这里是手动取出了一个ArticlePayService对象
            PayConfirmDTO confirm = SpringUtil.getBean(ArticlePayService.class)
                    .buildPayConfirmInfo(record.getId(), record);
            Context context = new Context();
            context.setVariable("vo", confirm);
            String confirmHtmlContent = springTemplateEngine
                    .process("PayConfirm", context);
            log.info("输出邮件内容: \n {} \n", confirmHtmlContent);
    ...
    @Override
    //传入第三方会回调的request和回调函数Function<input.class,return.class>
    public ResponseEntity<?> payCallback(HttpServletRequest request
            , Function<PayCallbackBo,Boolean> payCallback){
        //首先调用handler构造一个bo
    ...
    public ResponseEntity<?> refundCallback(HttpServletRequest request, Function<RefundNotification, Boolean> payCallback) {
        return thirdPayFacade.refundCallback(request,
                ThirdPayWayEnum.EMAIL, payCallback);
    }
}
